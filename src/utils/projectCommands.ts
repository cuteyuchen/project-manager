...
export function ensureNodeInstallCommand<T extends Pick<Project, 'type' | 'packageManager' | 'customCommands'>>(
  ...
  const customCommands = (Array.isArray(project.customCommands)
    ...
    : []).map((command) => {
      const isAutoGeneratedInstallCommand =
        command.command.trim() === installCommand &&
        (!!command.builtinId || AUTO_INSTALL_COMMAND_NAMES.has(command.name.trim()));

      if (!isAutoGeneratedInstallCommand) {
        return command;
      }
  ...
  const hasInstallCommand = customCommands.some((command) => command.command.trim() === installCommand);
